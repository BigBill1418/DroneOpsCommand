...
| `Register tunnel error ... Unauthorized: Invalid tunnel secret` after recreating cloudflared (public URL → CF 530) | The token in `.env` is stale. A long-lived container coasts on the env frozen at create time, so the drift only surfaces on `--force-recreate`/recreate. Fetch the current token — `GET https://api.cloudflare.com/client/v4/accounts/<account_id>/cfd_tunnel/<tunnel_id>/token` with an account-scoped API token (fleet: flat key `integrations.cloudflare.account_token` in `~/noc-master/data/secrets.local.yml`; decode the old token's base64 JSON for `a`=account / `t`=tunnel IDs) — update `.env`, recreate the service. Bit the demo stack 2026-06-11. |
